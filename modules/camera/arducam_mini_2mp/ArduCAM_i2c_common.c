...
LOG_MODULE_REGISTER(arducam_i2c, CONFIG_ARDUCAM_MODULE_LOG_LEVEL);

bool arducam_i2cInit(const struct arducam_conf *config)
{
  return arducam_i2cInit_pd(config);
}

void arducam_i2cStop(const struct arducam_conf *config)
{
  arducam_i2cStop_pd(config);
}
...
void arducam_OV2640_set_jpeg_quality(uint8_t value)
{
  ...
  arducam_wrSensorReg8_8(0x44, value); // JPEG //10-63 lower number means higher quality
}
